...
Time is injected via `TimeProvider` (already scaffolded in `infrastructure/time_provider.py`). The domain **never** calls `datetime.now()` directly.
...
- ADR-002: PostgreSQL persistence with transactional guarantees
- ADR-003: Database-level locking (SELECT FOR UPDATE)
- ADR-004: Capture history and audit trail
- ADR-005: Authorization expiry cleanup policy
